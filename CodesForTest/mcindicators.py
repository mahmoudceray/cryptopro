...
def readdf(coin, interval, path):
    df = pd.read_csv('{}\\DataFromBinance{}\\{}{}.csv'.format(path, interval, coin, interval), header=[0])
...
def WeisWaveVolume(df):
    ...
    return pd.concat([df, dfC], axis=1)
...
def plotchart(df, FirstCandle, LastCandle, coin, time, colums=1, c=1, heights1=9, heights2=3):
    ...
    fig.add_trace(go.Scatter(x=dfpl.index, y=dfpl['SUPERT_8_2.618'], line=dict(color='orange', width=1), name="ATR"), row=1, col=c)

    # add LINEAR REGRESSION  Long Trend trace
    fig.add_trace(go.Scatter(x=dfpl.index, y=dfpl.lts, line=dict(color='black', width=1), name="Long Trend"), row=1, col=c)

    # add LINEAR REGRESSION  Long Trend trace
    fig.add_trace(go.Scatter(x=dfpl.index, y=dfpl.Highlts, line=dict(color='red', width=2), name="High Long Trend"), row=1, col=c)

    # add LINEAR REGRESSION  Long Trend trace
    fig.add_trace(go.Scatter(x=dfpl.index, y=dfpl.Lowlts, line=dict(color='green', width=2), name="Low Long Trend"), row=1, col=c)


    # add Linear Regression Channel trace
    #line=dict(color='purple', width=1)

    fig.add_trace(go.Scatter(x=[dfpl.index[-150],dfpl.index[-1]] , y=[dfpl.y1RegCh2[-1], dfpl.y2RegCh2[-1]], mode="lines",name="RegChUp"), row=1, col=c)

    fig.add_trace(go.Scatter(x=[dfpl.index[-150],dfpl.index[-1]] , y=[dfpl.y1RegCh1[-1], dfpl.y2RegCh1[-1]], mode="lines",name="RegChMid"), row=1, col=c)

    fig.add_trace(go.Scatter(x=[dfpl.index[-150],dfpl.index[-1]] , y=[dfpl.y1RegCh0[-1], dfpl.y2RegCh0[-1]], mode="lines",name="RegChDown"), row=1, col=c)


    # add Support & Resistance Lines trace
    fig.add_trace(go.Scatter(x=dfpl.index, y=dfpl.ph1, line=dict(color='red', width=2), name="Support Line"), row=1, col=c)
    fig.add_trace(go.Scatter(x=dfpl.index, y=dfpl.pll, line=dict(color='blue', width=2), name="Resistance Line"), row=1, col=c)
    ...
    fig.add_trace(go.Bar(x=dfpl.index, y=dfpl["net asset volume"], marker_color= netvolumecolors ,showlegend=False), row=3, col=c)
    ...
    fig.add_trace(go.Bar(x=dfpl.index, y=dfpl["Volume"], marker_color=Volumecolors, showlegend=False), row=4, col=c)

    fig.update_yaxes(title_text="Volume", row=4, col=c)


    # Plot Buy VOLUME trace on 4nd row 
    fig.add_trace(go.Bar(x=dfpl.index, y=dfpl['BVOLUME'], marker_color='green', showlegend=False), row=5, col=c)
    fig.update_yaxes(title_text="B/S Volume", row=5, col=c)

    # Plot Sell volume trace on 4nd row 
    fig.add_trace(go.Bar(x=dfpl.index, y=dfpl['SVOLUME'], marker_color='red', showlegend=False), row=5, col=c)
    fig.update_yaxes(title_text="B/S Volume", row=5, col=c)
    ...
    fig.add_trace(go.Bar(x=dfpl.index, y=dfpl['Volatility'], marker_color=Volatilitycolors, showlegend=False), row=6, col=c)
    ...
    TTMhcolors = ['green' if dfpl["SQZPRO_13_2.0_13_2_1.5_1"][i] >= 0 else 'red' for i in range(len(dfpl))]
    fig.add_trace(go.Bar(x=dfpl.index, 
    ...
    fig.add_trace(go.Scatter(x=dfpl.index, y=dfpl.MFI, line=dict(color='black', width=2) ,showlegend=False), row=14, col=c)
    ...
    fig.add_trace(go.Scatter(x=dfpl.index, y=dfpl.wt1, line=dict(color='blue', width=2) ,showlegend=False), row=15, col=c)

    fig.add_trace(go.Scatter(x=dfpl.index, y=dfpl.wt2, line=dict(color='red', width=2) ,showlegend=False), row=15, col=c)
    ...
    fig.update_yaxes(title_text="Weis Wave Volume", showgrid=False, row=16, col=c)
...
def everythings(df, coin, interval):
    df = pd.DataFrame()
    ...
    #df = WeisWaveVolume(df)
   # levels(df)
   # plotchart(df,-153,-1,coin, time=interval)
